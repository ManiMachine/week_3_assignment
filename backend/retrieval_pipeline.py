...
class BM25Index:
    ...
    def search(self, query: str, top_k: int = 50) -> list[tuple[int, float]]:
        ...
        # 3. Get indices of top-k highest scores
        top_indices = np.argsort(scores)[-top_k:][::-1]  # Get last top_k and reverse
        
        # 4. Build result list with only non-zero scores
        ...
        # 5. Return results (already sorted by score due to argsort)
        return results
...
class RetrievalPipeline:
    ...
    def __init__(self, config: Optional[RetrievalPipelineConfig] = None):
        ...
        if not os.path.exists(config.chunks_path):
            raise FileNotFoundError(f"Chunks file not found: {config.chunks_path}")
        with open(config.chunks_path, "r", encoding="utf-8") as f:
            data = json.load(f)
            self.chunks = data["chunks"]  # Extract just the chunks list
        ...
        for r in bm25_results:
            ...
            if chunk_id in combined:
                ...
                combined[chunk_id]["combined_score"] = (
                    semantic_weight * combined[chunk_id]["semantic_score"]
